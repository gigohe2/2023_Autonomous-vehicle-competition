...
class RefTracker():
    ...
    def do(self, ego):
        ...
        rospy.loginfo(f'idx: {idx}, vel: {velocity}({ego.v:0.1f}), head: {ego.heading:0.1f}, theta: {theta:0.1f}, cte:{cte:0.1f}, steer:{steering:0.1f}')
    ...
    def set_ref_path(self, filename):
        ...
        filename = os.path.join(rospack.get_path('shark'), 
                                                 f'path/{filename}.csv')        
        self.ref_path = []
